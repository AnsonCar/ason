...
def dirTree(path, exclude_list=None, max_depth=None, current_depth=0, isNext=False, is_Next_last=False):
    # 获取给定路径下的文件和目录列表
    ...
    for i, file in enumerate(files):
        ...
        if os.path.isdir(os.path.join(path, file)) and (max_depth is None or current_depth < max_depth):
            ...
            dirTree(os.path.join(path, file), exclude_list, max_depth, current_depth + 1, isNext=True, is_Next_last=is_last)
